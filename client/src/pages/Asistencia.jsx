...
const Asistencia = () => {
  ...
  useEffect(() => {
    api.get('/materias/alumno/mis-materias').then(data => {
      setMaterias(data);
  ...
  const cargarAsistencias = async () => {
    ...
    setLoading(true);
    try {
      const data = await api.get(`/asistencias/mis-asistencias?materiaId=${materiaSeleccionada}`);
      setAsistencias(data);
      setPagina(1);
    } finally {
      setLoading(false);
  ...
  // Calcular totales sobre los filtrados
  ...
  // Badge de estado según tipo
  const renderEstadoBadge = (estado) => {
    switch (estado) {
      case 'presente':
        return <Badge bg="success" className="badge-asistencia">Asistencia</Badge>;
      case 'falta':
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">
        Consulta tu historial de asistencias por materia y rango de fechas.
      </p>
          ...
          <Col md={4}>
            <Form.Group>
              ...
              <Form.Control
                type="date"
                value={fechaInicial}
                onChange={(e) => { setFechaInicial(e.target.value); setPagina(1); }}
              />
            </Form.Group>
          </Col>
          <Col md={4}>
            <Form.Group>
              ...
              <Form.Control
                type="date"
                value={fechaFinal}
                onChange={(e) => { setFechaFinal(e.target.value); setPagina(1); }}
              />
            </Form.Group>
          </Col>
          <Col md={4}>
      ...
      <div className="tabla-asistencia">
        <div className="p-3 pb-0">
          <h5 className="fw-bold mb-3">{materiaActual?.nombre}</h5>
        </div>
          ...
          <Table responsive hover>
            <thead>
              <tr>
                <th>Fecha</th>
                <th>Estado</th>
                <th className="text-end">Observaciones</th>
              </tr>
            </thead>
            <tbody>
              ...
              {asistenciasPaginadas.length === 0 && (
                <tr>
                  <td colSpan={3} className="text-center text-muted py-4">Sin registros en este período.</td>
                </tr>
              )}
            </tbody>
          </Table>
      ...
      <Row className="mt-4">
        <Col md={4} className="mb-3">
          <div className="total-card">
            <span className="total-label">Total Asistencias</span>
            <span className="total-value success">{totalAsistencias}</span>
          </div>
        </Col>
        <Col md={4} className="mb-3">
          <div className="total-card">
            <span className="total-label">Total Faltas</span>
            <span className="total-value danger">{totalFaltas}</span>
          </div>
        </Col>
        <Col md={4} className="mb-3">
          <div className="total-card">
            <span className="total-label">Total Retardos</span>
            <span className="total-value warning">{totalRetardos}</span>
          </div>
        </Col>
      </Row>
    </div>
