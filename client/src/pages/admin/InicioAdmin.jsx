...
const InicioAdmin = () => {
  const [stats, setStats] = useState({
    alumnos: 0, profesores: 0, materias: 0,
    reportesPendientes: 0, lecturasPendientes: 0, evaluaciones: 0
  });
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [users, materias, reportes, lecturas, evaluaciones] = await Promise.all([
          api.get('/users'),
          api.get('/materias'),
          api.get('/reportes'),
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">
        Gestiona usuarios, materias y configuraciones del sistema universitario.
      </p>
          ...
          <Row className="mb-4">
            <Col sm={4} className="mb-3">
              <StatCard
                ...
                color={stats.reportesPendientes > 0 ? '#ca8a04' : '#6c757d'}
              />
            </Col>
            <Col sm={4} className="mb-3">
              <StatCard
                icon={ClipboardCheck}
                value={stats.lecturasPendientes}
                label="Lecturas pendientes"
                color={stats.lecturasPendientes > 0 ? '#ca8a04' : '#6c757d'}
              />
            </Col>
            <Col sm={4} className="mb-3">
